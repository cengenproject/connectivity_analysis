function [sig_genes,beta,pval,tstat]=expression_test_neuron_partners_vs_adjacent(neuron_1,feasible_connectome,G,gene_set,neuron_set,connectome,weighted_connectome,withself,unique,normalization,numhetero)
if nargin<11
    withself=1;
    unique=0;
    normalization=0;
...
j=find(strcmpi(neuron_set,neuron_1));neuron_2=neuron_1;
partners_1 = find(connectome(i,:));
partners_2 = setdiff(find(feasible_connectome(i,:)),partners_1);
...
plot([1 size(G,2)],[length(partners_1)-length(partners_common)+0.5 length(partners_1)-length(partners_common)+0.5],'m--','LineWidth',2);
plot([1 size(G,2)],[length(partners_1)+length(partners_common)+0.5 length(partners_1)+length(partners_common)+0.5],'m--','LineWidth',2);
end
...
title('Homophilic CAMs -- above red line: neuron+synaptic partners, below red line: neuron + adjacent partners');
...
title('Top 20 Heterophilic CAMs -- above red line: neuron+synaptic partners, below red line: neuron + adjacent partners');
